...
; ;Does this shirt match these trousers ? (TROUSERS)
 ...
 (test (and (neq ?vib 0) (neq ?vib -)))
  =>
       	(retract ?f0)
	(bind ?hmng (remove_character "_" ?hmng " "))
       	(printout ?*op_fp5* "(id-Apertium_input " ?id " "(implode$ ?hmng)" "?vib ")" crlf)
...
; ;There was a marked difference in the prices of dishes .
 ...
 ?f0<-(id-HM-source ?id ?hmng WSD_compound_phrase_word_mng|Database_compound_phrase_word_mng|WSD_verb_phrase_word_mng|WSD_word_mng|Idiom_word_mng|provisional_Database_compound_phrase_word_mng|Template_word_mng)
  =>
       	(retract ?f0)
	(bind ?hmng (remove_character "_" ?hmng " "))
       	(printout ?*op_fp5* "(id-Apertium_input " ?id " "(implode$ ?hmng) ")" crlf)
...
;  ;-------------------------------------------------------------------------------------------------------------------------
  ...
  (pada_info (group_cat PP) (group_ids $?ids ?id) (vibakthi ?vib)(number ?num)(case ?case)(gender ?gen)(person ?p) )
...
;  (test (neq ?vib 0))
  ...
  =>
        (retract ?f1)
;<fs af='mEM,pn,m,sg,1,d,,0'>
	(printout ?*A_fp5* "1." ?id"	" ?h_word "	NN	<fs af='"?h_word",n,"?gen","?tn","?tp","?case","?vib",0'>" crlf)
...
;-------------------------------------------------------------------------------------------------------------------------
  (defrule VP_rule_with_vib_hid
  (declare (salience 350))
  (pada_info (group_cat VP) (group_ids $?ids ?id) (H_tam ?vib)(number ?num)(case ?case)(gender ?gen)(person ?p) )
  ?f1<-(id-HM-source ?id ?h_word&~- ?)
;  (test (neq ?vib 0))
  (id-cat_coarse ?id ?cat)
   (person ?p ?tp)
  ...
  =>
        (retract ?f1)
;<fs af='mEM,pn,m,sg,1,d,,0'>
	(printout ?*A_fp5*  "1."?id"	" ?h_word "	VP	<fs af='"?h_word",v,"?gen","?tn","?tp","?case","?vib",0'>" crlf)
;        (printout ?*A_fp5* "(id-Apertium_input "?id " ^"?h_word "<cat:n><case:"?case"><gen:"?gen"><num:"?num">$  ^" ?vib "<cat:prsg>$)"  crlf)
	(printout ?*aper_debug-file* "(id-Rule_name  "?id "  PP_rule_with_vib_hid )" crlf)
  )
;  ;-------------------------------------------------------------------------------------------------------------------------
  ...
  (pada_info (group_cat PP) (group_ids $?ids)(number ?num)(case ?case)(gender ?gen)(person ?p) )
 ...
 =>
	(retract ?f2)
	(if (eq ?cat noun) then
		(printout ?*A_fp5* "1."?id1"	" ?h_word "	NN	<fs af='"?h_word",n,"?gen","?tn","?tp","?case",0,0'>" crlf)
	else
		(printout ?*A_fp5* "1."?id1"	" ?h_word "	NN	<fs af='"?h_word",adj,"?gen","?tn","?tp","?case",0,0'>" crlf)
	)
