...
(if ?*debug_flag* then
(printout wsd_fp "(dir_name-file_name-rule_name-id-wsd_root_mng  " ?*wsd_dir* " roll.clp roll0 " ?id "  Dulaka )" crlf)) 
)
...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id luDaka));Automatically modified 'affecting_id-affected_ids-wsd_group_root_mng ?id ?id1' to 'id-wsd_root_mng ?id ' by Sukhada's program. 
(if ?*debug_flag* then
(printout wsd_fp "(dir_name-file_name-rule_name-id-wsd_root_mng  " ?*wsd_dir* " roll.clp roll2 " ?id "  luDZaka )" crlf)) 
)
...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id luDaka))
(if ?*debug_flag* then
