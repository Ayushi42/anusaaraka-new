...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id kisI_burI_cIja_ko_svayaM_se_xUra_raKa));Automatically modified 'affecting_id-affected_ids-wsd_group_root_mng ?id ?id1' to 'id-wsd_root_mng ?id ' by Sukhada's program. 
(if ?*debug_flag* then
(printout wsd_fp "(dir_name-file_name-rule_name-id-wsd_root_mng  " ?*wsd_dir* " stave.clp stave0 " ?id "  kisI_burI_cIjZa_ko_svayaM_se_xUra_raKa )" crlf)) 
)
...
=>
(retract ?mng)
(assert (affecting_id-affected_ids-wsd_group_root_mng ?id ?id1 kisI_burI_cIja_ko_svayaM_se_xUra_raKa))
(if ?*debug_flag* then
...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id koI_motI_CadI_yA_daNdA))
(if ?*debug_flag* then
