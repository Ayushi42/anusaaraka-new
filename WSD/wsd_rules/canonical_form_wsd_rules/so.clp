;### Counter ex: Why AK47 is called so?
;AK47 ko EsA kyoM kahA jAwA hE?
(defrule so0
...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id isalie))
(if ?*debug_flag* then
...
;@@@ Added by Roja(13-06-14). Suggested by Chaitanya Sir.
;Why AK47 is called so?
;AK47 ko EsA kyoM kahA jAwA hE?
(defrule so_last_wrd
(declare (salience 5000))
(id-root ?id so)
...
(if ?*debug_flag* then
(printout wsd_fp "(dir_name-file_name-rule_name-id-wsd_root_mng   " ?*wsd_dir* "  so.clp       so_last_wrd   "  ?id "  EsA )" crlf))
)

;@@@ Added by Roja(13-06-14). Suggested by Chaitanya Sir.
;I think so.
(defrule so_last_wrd1
(declare (salience 5000))
(id-root ?id so)
?mng <-(meaning_to_be_decided ?id)
(id-last_word ?id so)
...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id EsA_hI))
(if ?*debug_flag* then
(printout wsd_fp "(dir_name-file_name-rule_name-id-wsd_root_mng   " ?*wsd_dir* "  so.clp       so_last_wrd1   "  ?id "  EsA_hI)" crlf))
)
...
;kyA vaha kala A rahI hE ? mEM EsA hI socawI hUz.
