...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id kIcada))
(if ?*debug_flag* then
...
=>
(retract ?mng)
(assert (id-wsd_root_mng ?id kisI_waraha_saPala_ho));Automatically modified 'affecting_id-affected_ids-wsd_group_root_mng ?id ?id1' to 'id-wsd_root_mng ?id ' by Sukhada's program. 
(if ?*debug_flag* then
(printout wsd_fp "(dir_name-file_name-rule_name-id-wsd_root_mng  " ?*wsd_dir* " mud.clp mud1 " ?id "  kisI_waraha_saPala_ho )" crlf)) 
)
