...
(not (id-Apertium_output ? ?mng1 ?mng2))
=>
...
(database_info (components ?mng&ke|kI|se|meM|para ?mng1))
(manual_id-word ?id ?mng)
(manual_id-word ?id1&:(=(+ ?id 1) ?id1) ?mng1)
(not (mng_has_been_grouped ?id1))
(not (chunk_name-chunk_ids VGF $? ?id1 $?))
...
(not (mng_has_been_grouped ?id2))
(not (mng_has_been_grouped ?id3))
(not (database_info (meaning ?w0)(group_ids ?aid)))
=>
...
(declare (salience 570))
?f1<-(manual_word_info (head_id ?mid) (word ?p&isake|isakI|usake|inake|isa|isI|apane|jisake|wumhAre|Apake|sabake|unake|jisa))
?f<-(manual_word_info (head_id ?h) (word ?w&awirikwa|prakAra|waraha|vajaha)(group_ids ?mid1&:(=(+ ?mid 1) ?mid1) $?ids))
=>
	(retract ?f1)
	(modify ?f  (word ?p ?w)(group_ids ?mid ?mid1 $?ids))
)
;----------------------------------------------------------------------------------------------------------
;Added by Shirisha Manju 
