...
(left_over_ids $? ?id $?)
(manual_id-word ?id ?w) ;eka bAra niyawa parisampawwiyoM para [Karca kI gaI] niXi ko vApasa nahIM liyA jAwA Ora xUsare para nahIM lagAyA jA sakawA hE
(not (hlist $?))
=>
...
;---------------- generate correct and wrong word list for count diff program --------------- 
...
(test (eq (integerp (member$ ?m (create$ hE hEM howA hE howI hE howe hEM hogA avaSya nahIM jarUra sakawI))) FALSE))
?f1<-(left_over_ids $?ids)
...
(declare (salience 41))
(or (group_ids-start_id-end_id $?d $?ids - ? ?)(group_ids-start_id-end_id $?ids $?d - ? ?))
(test (neq (length $?d) 0))
...
(defrule get_mwe_parser_grop_ids
(declare (salience 30))
